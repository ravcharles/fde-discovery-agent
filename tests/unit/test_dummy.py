...
async def test_discovery_loop_continue(mock_generate):
    mock_json = '{"refined_draft": {"context": "Context details", "quantify": "Quantify details", "scope": "Scope details"}, "customer_confidence": "High", "customer_accuracy": "Cause-and-Impact", "finalize": false, "next_question": "How soon do you need this?"}'
    mock_generate.return_value = MockResponse(mock_json)
    ...
    called_contents = mock_generate.call_args[1]["contents"]
    assert "Probing Strategies (Slide 5)" in called_contents
    assert "Reference Scoping Questions (Slide 4)" in called_contents


@pytest.mark.asyncio
@patch("app.agent.client.models.generate_content")
async def test_discovery_loop_finalize(mock_generate):
    mock_json = '{"refined_draft": {"context": "Context details", "quantify": "Quantify details", "scope": "Scope details"}, "customer_confidence": "High", "customer_accuracy": "Justification-focused", "finalize": true, "next_question": ""}'
    mock_generate.return_value = MockResponse(mock_json)
...
async def test_why_change_start_node(mock_generate):
    mock_json = '{"refined_planner": {"target": "Retail enterprises", "business_goals": "goals", "known_needs": "known", "unconsidered_needs": "unconsidered", "trends_changes": "trends", "previous_approach": "prev", "consequences": "cons", "new_way": "newway", "likely_outcomes_project": "project", "likely_outcomes_business_unit": "bu", "likely_outcomes_corporate": "corporate"}, "primary_bias": "Preference Stability", "status_quo_analysis": "analysis", "rubric_evaluation": "Sufficient", "finalize": false, "next_question": "Provocative trend question?"}'
    mock_generate.return_value = MockResponse(mock_json)
...
async def test_why_change_start_node_fallback_on_tool_failure(mock_generate):
    ...
    # Configure mock to fail (empty string) on first call, and succeed on second (fallback) call
    mock_generate.side_effect = [MockResponse(""), MockResponse(mock_json)]
...
async def test_why_change_loop_continue(mock_generate):
    mock_json = '{"refined_planner": {"target": "Retail", "business_goals": "goals", "known_needs": "known", "unconsidered_needs": "unconsidered", "trends_changes": "trends updated", "previous_approach": "prev", "consequences": "cons", "new_way": "newway", "likely_outcomes_project": "project", "likely_outcomes_business_unit": "bu", "likely_outcomes_corporate": "corporate"}, "primary_bias": "Preference Stability", "status_quo_analysis": "analysis", "rubric_evaluation": "Sufficient", "finalize": false, "next_question": "Provocative question 2?"}'
    mock_generate.return_value = MockResponse(mock_json)
...
async def test_full_workflow_path(mock_generate):
    ...
    discovery_json_continue = '{"refined_draft": {"context": "Context details", "quantify": "Quantify details", "scope": "Scope details"}, "customer_confidence": "High", "customer_accuracy": "Cause-and-Impact", "finalize": false, "next_question": "Second question?"}'
    discovery_json_finalize = '{"refined_draft": {"context": "Context details", "quantify": "Quantify details", "scope": "Scope details"}, "customer_confidence": "High", "customer_accuracy": "Justification-focused", "finalize": true, "next_question": ""}'
    agreement_json = '{"agreed": true, "edits": ""}'
    why_change_json_continue = '{"refined_planner": {"target": "Retail", "business_goals": "goals", "known_needs": "known", "unconsidered_needs": "unconsidered", "trends_changes": "trends", "previous_approach": "prev", "consequences": "cons", "new_way": "newway", "likely_outcomes_project": "project", "likely_outcomes_business_unit": "bu", "likely_outcomes_corporate": "corporate"}, "primary_bias": "Preference Stability", "status_quo_analysis": "analysis", "rubric_evaluation": "Sufficient", "finalize": false, "next_question": "Why change question 2?"}'
    why_change_json_finalize = '{"refined_planner": {"target": "Retail", "business_goals": "goals", "known_needs": "known", "unconsidered_needs": "unconsidered", "trends_changes": "trends", "previous_approach": "prev", "consequences": "cons", "new_way": "newway", "likely_outcomes_project": "project", "likely_outcomes_business_unit": "bu", "likely_outcomes_corporate": "corporate"}, "primary_bias": "Preference Stability", "status_quo_analysis": "analysis", "rubric_evaluation": "Excellent", "finalize": true, "next_question": ""}'
    
    mock_generate.side_effect = [
        MockResponse(triage_json),              # triage_and_draft
        MockResponse(discovery_json_continue),  # discovery_loop (run 1)
        MockResponse(discovery_json_finalize),  # discovery_loop (run 2)
        MockResponse(agreement_json),           # process_agreement
        MockResponse(why_change_json_continue), # why_change_start_node
        MockResponse(why_change_json_continue), # why_change_loop (run 1)
        MockResponse(why_change_json_finalize), # why_change_loop (run 2)
        MockResponse(agreement_json),           # process_wc_agreement
        MockResponse("Final markdown summary report detailing realign mapping and why change pitch.") # reasoning_summary_node
    ]
